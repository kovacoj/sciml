...
def _param_direction(model, rng, scale=1.0):
...
def _apply_direction(model, direction, scale):
    """Add scale * direction to model parameters."""
    saved = []
...
def gradient_check(
    ...
    output_layer_only: if True, only perturb the final layer parameters
    (useful when the model is zero-initialized and hidden layers have
    ...
    for dir_idx in range(n_directions):
        if output_layer_only:
            # only perturb the last layer
            params = list(model.parameters())
            last_param = params[-1]
            d = rng.standard_normal(last_param.shape) * 1e-4
            d_tensor = torch.from_numpy(d).to(dtype=last_param.dtype,
                                             device=last_param.device)
            direction = [torch.zeros_like(p) for p in params]
            direction[-1] = d_tensor
        else:
