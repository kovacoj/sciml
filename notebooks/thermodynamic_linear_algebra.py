...
def _(mo):
    n_iter_slider = mo.ui.slider(
        10_000, 100_000, 10_000, value=10_000, label="Number of steps"
    )
    beta_slider = mo.ui.slider(
        0.01, 2, 0.01, value=0.1, label=r"$\beta$"
    )
    gamma_slider = mo.ui.slider(
        1, 50, 0.5, value=10.0, label=r"$\gamma$"
    )
    dt_slider = mo.ui.slider(
        0.001, 0.5, 0.001, value=0.01, label=r"$\delta t$"
    )
...
def _(A, beta_slider, dt_slider, gamma_slider, n_iter_slider, np):
    ...
    for _i in range(1, n_iter):
        x[_i] = (
            ...
            - 1 / gamma * A @ x[_i - 1] * dt
            + np.random.normal(
                0, np.sqrt(2 / (gamma * beta) * dt), size=(2,)
            )
        )
